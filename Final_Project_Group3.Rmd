...
### Relationship between ProductRelated and Revenue
...
Looking at the Product Related attribute against the target variable, it can be concluded that users that visit more Product Related pages are likely to finalize a purchase.
...
The correlation between the Special day and the target variable is negative. Note that a “0” value is assigned to dates visited well before or after the Special day, and non-zero values are assigned to the dates visited closest to the Special day, wherein 1 would mean the user visited on a date very close to the Special day.  From this graph, one may infer the e-commerce website was visited the most on dates well before or after the Special day (regular days of the year). Also, the portion of the users visiting during this time frame that did make a purchase, are greater than the portion of users making purchases closer to the Special day. This might be explained by people taking into account delivery time and purchasing so that an item arrives exactly on that Special day.
...
Users visiting from different geographic regions appear to show similar shopping behaviour. There are 9 different regions; however they were not defined in the reference article (Sakar et al., 2019).
...
In sum, the following were used in the prediction models: Page Values, Month, Product-related, Produce-related duration, Administrative, Visitor type, Informational, Administrative Duration, Informational Duration, Weekend, Browser, Special Day, Bounce Rates and Exit Rates.
...
‘ROSE’ R package library was used to address the unbalanced data, since it creates balanced synthetic data samples through oversampling, which is accomplished by enlarging the features space of minority and majority class examples (R Documentation, n.d.-b). Operationally, the new examples are drawn from a conditional kernel density estimate of the two classes, as described in Menardi and Torelli, 2013.

The dataset was split into 3 subsets for predictive modeling: train, validate, and test. In addition to ‘ROSE’ R package, createDataPartition function was also used to address the unbalanced data and to ensure that the target variable is equally and consistently represented in all three data subsets (the 3 tables below show that the target variable 0's and 1's are equally and consistently represented in the train, validate, and test datasets, respectively).
...
plot(perf_lr,color="red", main="Logistic Regression")
auc_lr<-auc(test_d$Revenue,pred.test_lr)
metrics["AUC","LogisticRegression"]=auc_lr
```

Logistic Regression had a mean prediction score of 81%, and had the following metrics along with the above ROC curve:
...
plot(perf_rf,color="red",main="Random Forest")
metrics["AUC","RandomForest"]=auc(test_d$Revenue,pred.test_d.rf)
```

Random Forest model had the following metrics and the above ROC curve:
...
However, k-NN displayed contradicting results. It displayed Browser type and Visitor type as the important features, while those two features were least important features in other models. However, k-NN still showed Page value as one of the top important features, which is consistent with other models as well.
...
For the Page value, the interquartile range of cluster 2 is higher than the cluster 1. The observations made thus far seem to suggest that the site-visitors in cluster 2 are more likely to make purchases than the site-visitors in cluster 1.
...
The site-visitors in cluster 2 (the ones with higher likelihood of making purchases) are likely to shop much more in November than site-visitors in cluster 1. From this, one can speculate that site-visitors who are frequent buyers on e-commerce websites, are more likely to prepare for winter holidays seasons by shopping for gifts.
...
Moreover, the following shows the differences between the clusters' centers.
...
The following pie charts show the cluster representation of 1 and 2, respectively. The pie charts indicate the interests of e-commerce visitors, such as Product Related pages and Administrative pages of the shopping website.

```{r, echo=FALSE}
#cluster 1
pie(colSums(clust_data[k2$cluster==1,]),cex=1,main = "Cluster 1")
#cluster 2
pie(colSums(clust_data[k2$cluster==2,]),cex=1,main = "Cluster 2")
```

The above pie charts indicate that there are differences in interests between cluster 1 and 2. Cluster 2 overall has more diverse interests than cluster 1, because cluster 2’s pie chart has many more bigger pieces than cluster 1. Furthermore, cluster 2 shows bigger interest in spending more time in web pages that are related to administration (e.g. account  setup and management). The same cluster 2 also shows greater interest in spending more time in web pages relevant to the company's information (e.g. company's founder, address). Overall, it seems that cluster 1 does not have much varied interest compared to cluster 2.
...
Here, cluster 2 visits more Administrative web pages, and spends more time in those web pages compared to cluster 1.
...
grid.arrange(pamk2plot3, pamk2plot4, pamk2plot5, pamk2plot6, nrow=2, ncol=2)
```

Cluster 1 and 2 repeat the same behaviour trend with Informational and Product Related web pages as well, where cluster 2 visits more frequently and spends more time in the web pages than cluster 1.
...
These graphs illustrate that cluster 2 has lower Bounce and Exit rates than cluster 1. Combining this observation with the last observations from the above plots, this seems to indicate that cluster 2 is likely to purchase and cluster 1 is unlikely to purchase.

```{r, echo=FALSE}
#Explaining clusters by Page Values
ggplot(pamdata, aes(factor(pam.res$clustering),y=PageValues)) +
  geom_boxplot(aes(fill=factor(pam.res$clustering)))+ggtitle("Page Values")
```

Cluster 2 (likely to make a purchase) shows higher Page value in its interquartile range than compared to cluster 1. This further supports the idea that cluster 2 represents a segment that is likely to make a purchase.

```{r, echo=FALSE}
#Explaining clusters by Months visits
ggplot(data = pamdata, mapping = aes(x = as.factor(pam.res$clustering), fill = as.factor(Month))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Month")
```

Cluster 2 is likely to shop more near the end of the year, such as November. Cluster 1 is likely to shop more near the beginning and the earlier months of the year, such as February~April. Cluster 1 shops significantly less in November compared to cluster 2.
...
The Operating System of type 2 and Browser of type 2 are the most popular and widely used across the two clusters. However, cluster 2 is more likely to use this popular Operating System and Browser type compared to cluster 1.

```{r, echo=FALSE}
#Explaining clusters by Traffic Types
ggplot(data = pamdata, mapping = aes(x = as.factor(pam.res$clustering), fill = as.factor(TrafficType))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Traffic Types")
```

Traffic type 3 is the most popular traffic type that motivates users to the shopping website. Similarly to the Operating System and Browser type, cluster 2 is more likely to use this popular Traffic type compared to cluster 1.

```{r, echo=FALSE}
#Explaining clusters by Visitor Types
ggplot(data = pamdata, mapping = aes(x = as.factor(pam.res$clustering), fill = as.factor(VisitorType))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Visitor Types")
```

Both clusters are very unlikely to be Returning Visitors, as shown by the miniscule red bar that represents the Returning Visitor type in the above plot. Compared to cluster 1, cluster 2 is more likely to contain New Visitors, while cluster 1 is more likely to consist of "Other" Visitor type.
...
grid.arrange(pamk2plot15, pamk2plot12, nrow=2)
```

According to graphs above, two clusters were very much similar in these features.
...
The pie charts below show the cluster representation of 1 and 2, respectively, in the PAM model.

```{r, echo=FALSE}
#Viewing cluster 1
#subset(pamdata,pam.res$clustering==1)
pie(colSums(clust_data[pam.res$clustering==1,]),cex=1, main = "Cluster 1")

#Viewing cluster 2
#subset(pamdata,pam.res$clustering==2)
pie(colSums(clust_data[pam.res$clustering==2,]),cex=1, main = "Cluster 2")
```

The above pie charts show that cluster 1 has more interest in Administrative related web pages than cluster 2. Both clusters are most interested in spending time in the Product Related pages, as shown by the largest area in the pie chart. This trend is consistent in both K-means and PAM models. Overall, cluster 1 shows more varied and diverse interests than cluster 2, shown by the bigger pieces dedicated to other features in the cluster 1 pie chart.
...
The following graph shows the model’s cluster dendrogram and cluster plot. The model placed 6,946 site-visitors in cluster 1, and 5,384 site-visitors in cluster 2.
...
grid.arrange(hclustk2plot1, hclustk2plot2, nrow=2)
```

It can be observed from the above plot that cluster 2 in this hierarchical model visits more administration websites, and also spends more time in the administration websites than cluster 1.
...
grid.arrange(hclustk2plot3, hclustk2plot4, hclustk2plot5, hclustk2plot6, nrow=2, ncol=2)
```

The same behaviour trend is visible with regards to Informational and Product Related websites as well, where cluster 2 visited more frequently and stayed longer in the websites.
...
Also, the cluster 2 had lower Bounce and Exit rates compared to cluster 1.

```{r, echo=FALSE}
#Explaining clusters by Page Values
ggplot(hclustdata, aes(factor(groups_k2),y=PageValues)) +
  geom_boxplot(aes(fill=factor(groups_k2)))+ggtitle("Page Values")
```

Cluster 2 also had a higher Page value than cluster 1, which is a consistent trend due to its low Bounce and Exit rate. At this point, it can be anticipated that cluster 2 represents those who are likely to purchase, whereas cluster 1 ones who are unlikely to make a purchase on shopping websites.

```{r, echo=FALSE}
#Explaining clusters by Months visits
ggplot(data = hclustdata, mapping = aes(x = as.factor(groups_k2), fill = as.factor(Month))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Month")
```

Compared to cluster 1, cluster 2 is more likely to shop around November. On the other hand, cluster 1 shops more around mid-early year, such as April and May, compared to cluster 2.
...
Cluster 1 and 2 used different Browsers and Operating Systems. Cluster 1 prefers Operating System types as 1 and 3, while cluster 2 prefers Operating System type 2. For a Browser type, cluster 1 prefers type 1, while cluster 2 prefers a Browser type 2. This difference in the preference of Operating System and Browser type was also observed in previous clustering models.

```{r, echo=FALSE}
#Explaining clusters by Traffic Types
ggplot(data = hclustdata, mapping = aes(x = as.factor(groups_k2), fill = as.factor(TrafficType))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Traffic Type")
```

For a Traffic type, it can be observed that cluster 2 especially uses a Traffic type 2 to arrive at the shopping website, whereas cluster 1 uses a Traffic type 4 more.

```{r, echo=FALSE}
#Explaining clusters by Visitor Types
ggplot(data = hclustdata, mapping = aes(x = as.factor(groups_k2), fill = as.factor(VisitorType))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Visitor Type")
```

From the above plot, cluster 2 is likely to be a new visitor to the website, while cluster 1 is likely to be "Other" visitor type. It is interesting to note that both clusters are unlikely to be a Returning visitor type.
...
grid.arrange(hclustk2plot13, hclustk2plot16, nrow=2)
```

Similar to previous observations, Region and Weekend were consistent for both clusters.

### Model Evaluation - Clustering, k=2

To compare which models performed better in clustering for k=2, two metrics were used: internal and external. Silhouette is an internal evaluation method, where the score ranges from -1 to 1. The closer a score is to 1, the better the model is. Also, the Silhouette method produces a visual plot that shows a “tail” that may run beneath the x-axis. The longer this tail is, and deeper it runs beneath the x-axis, the worse the model is, since that tail represents the amount of data points that are misplaced. For the external evaluation method, Rand index score was used. The Rand index, named after William M. Rand, is a measure to compare the similarity of two clustering outcomes by giving a value between 0 and 1, where 1 means the two clustering outcomes match identically (R Documentation, n.d.-a; Wikipedia, 2020).
...
* k=2 misplaced observations: 997
* Rand Index: 0.653
...
* k=2 misplaced observations: 1,172
* Rand Index: 0.584
...
* k=2 misplaced observations: 2,561
* Rand Index: 0.523
...
metrics_k2_clustering["k=2 misplaced observations","kmeans"]=0.997 #note: scaled
metrics_k2_clustering["k=2 misplaced observations","pam"]=1.172 #note: scaled
metrics_k2_clustering["k=2 misplaced observations","Hclust"]=2.561 #note: scaled
...
Overall, it seems like the PAM model achieved relatively higher performance as it is depicted by evaluation metrics.
...
While clustering algorithms are now proven to be less effective for predicting user purchase intention using this dataset, they look promising to be utilized as a method to research and understand online users’ behaviours and trends.

### Further Research: k=3, Clustering, K-means, PAM, and Hierarchical

To further analyze the site-visitors and their behaviour trends, k=3 was used for all the clustering models again.
...
As in k=2 clustering, the k=3 clustering also produced a group that was likely and unlikely to make a purchase. However, it further divided the group that was unlikely to make a purchase into two. While both of those two groups were unlikely to make a purchase (performed similarly in page visit and duration features), one cluster had a slightly lower Bounce and Exit rates. Therefore, it seemed that k=3 clustering divided the site-visitors into a) likely to make purchases (cluster 1), b) less likely to make a purchase (cluster 3), and c) very stubborn and least likely to make a purchase (cluster 2).
...
The graph above demonstrates that the cluster 2 visited Administrative web pages the least, while cluster 1 visited the most.
...
Moreover, cluster 1 spent the most time on the Administrative web pages, cluster 2 spent the least time, and cluster 3 was in-between.
...
Similar observations were detected when clusters compared with respect to Informational and Product Related web pages.
...
The cluster 2 (least likely to make a purchase) also showed the highest Bounce and Exit rate, while the cluster 3 showed the least, and the cluster 1 was, again, in-between.
...
An interesting observation is discovered among the site-visitors who are least likely to purchase (cluster 2). They visit earlier in the year (February~May) compared to other clusters, while other clusters visit more in the later months of the year (November) when winter holiday is close. It is unclear why early months of the year are popular for the site-visitors in cluster 2. Nevertheless, e-commerce business owners may use this information to come up with promotions to motivate cluster 2 visitors habits to change from not purchasing to purchasing during their visit from February to May.
...
Similar to the previous trends observed, clusters did not differ much in Region.

For visitor type, cluster 2 is most likely to be the New visitor to the website, while cluster 3 is most likely to be the "Other" type in the visitor type. All three clusters are very unlikely to be Returning visitors to the website.
...
The bar plot shows that cluster 2 shops less on weekends in contrast to other two clusters.

The following table shows the k-mean centers for cluster 1, 2, and 3.
...
Next, cluster representations are shown in the following pie charts for cluster 1, 2, and 3, respectively, for k=3 K-means clustering model.

```{r, echo=FALSE}
pie(colSums(clust_data[k3$cluster==1,]),cex=1, main = "Cluster 1")
pie(colSums(clust_data[k3$cluster==2,]),cex=1, main = "Cluster 2")
pie(colSums(clust_data[k3$cluster==3,]),cex=1, main = "Cluster 3")
```

The pie charts show that cluster 2, which is the cluster least likely to make a purchase, has the most diverse and varied interests. Cluster 1 and 3 pie charts are very similar, but also have some differences. For instance, cluster 1 has bigger interest in Informational pages than cluster 3. Also, cluster 3 seems to show bigger pie chart pieces for other features than cluster 1, which may indicate that cluster 3 has more diverse interest than cluster 1. Overall, it was cluster 2 that had the greatest and the most varied interests out of all three clusters.
...
Here, cluster 3 visits the Administrative web pages the most frequently, and also spends the most time in those web pages. Cluster 2 visits the second most frequently, and cluster 3 visits the least. Cluster 2 and 3 spends almost the same time in the web pages, where cluster 1 spends a little bit more time than cluster 2. Overall, cluster 3 clearly has the highest number of web page visits, and amount of time spent in those web pages.
...
grid.arrange(pamk3plot3, pamk3plot4, pamk3plot5, pamk3plot6, nrow=2, ncol=2)
```

The exact same behaviour patterns were observed for the Informational and Product Related pages, where cluster 3 visited and spent time the most in the related sites, and cluster 1 visited and spent time the least in those sites. It is interesting to note that the three clusters behaved the same with respect to the Informational web pages, and totally different on Product Related pages.
...
Cluster 3 had the lowest Bounce and Exit rate, while cluster 1 had the highest. At this point, it can be speculated that cluster 3 is likely to make a purchase, and cluster 1 is the least likely to make a purchase, while cluster 2 is in-between.

```{r, echo=FALSE}
#Explaining clusters by Page Values
ggplot(pamresearchdata, aes(factor(pam.res_k3$clustering),y=PageValues)) +
  geom_boxplot(aes(fill=factor(pam.res_k3$clustering)))+coord_flip()+ggtitle("Page Values")
```

Cluster 1 had the lowest Page value, which is consistent with the observation that it is the least likely to make a purchase. However, it's interesting to mention that both clusters 2 and 3 had similar Page values.

```{r, echo=FALSE}
#Explaining clusters by Months visits
ggplot(data = pamresearchdata, mapping = aes(x = pam.res_k3$clustering, fill = as.factor(Month))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Month")
```

Cluster 3 is most likely to shop near the end of the year, especially in November, while cluster 1 is more likely than other clusters to shop earlier in the year, specifically in February. Cluster 2 was in-between. It is worth noting that July~August is an unpopular time period for shopping for all three clusters.
...
Operating System type 2 and Browser type 2 are the most widely used and popular ones. This observation was derived in previous models as well. Cluster 3 is most likely to use that popular Operating System and Browser type, while cluster 1 is more likely than other clusters to use unpopular Operating System and Browser types.

```{r, echo=FALSE}
#Explaining clusters by Visitor Types
ggplot(data = pamresearchdata, mapping = aes(x = pam.res_k3$clustering, fill = as.factor(VisitorType)))+geom_bar(position="fill")+ylab("Frequency")+ggtitle("Visitor Type") #+facet_wrap(pam.res_k3$clustering)+ coord_polar("y", start=0)
```

While all three clusters were most likely to be New visitors and least likely to be Returning visitors to the website, cluster 3 is the more likely than other clusters to be a New visitor type. Meanwhile, cluster 1 is more likely than other clusters to be the "Other" visitor type. This trend was replicated in previous models as well.

```{r, echo=FALSE}
#Explaining clusters by Traffic Types
ggplot(data = pamresearchdata, mapping = aes(x = pam.res_k3$clustering, fill = as.factor(TrafficType))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Traffic Type")
```

Traffic type 2 is the most widely used and popular Traffic type that motivates online shoppers to arrive at the shopping website. Cluster 3, out of all three, is the most likely to use this popular Traffic type. On the other hand, cluster 1 is the least likely to use this popular Traffic type, and most likely to use other Traffic types. Cluster 2 was in-between. This observation was also revealed in previous models as well.
...
grid.arrange(pamk3plot13, pamk3plot16, nrow=2)
```

Similar to observations made in previous models, distributions of region and weekend categories are the same in all three clusters.

The following pie charts show cluster demarcation for the PAM model, k=3.

```{r, echo=FALSE}
pie(colSums(clust_data[pam.res_k3$clustering==1,]),cex=1, main = "Cluster 1")
pie(colSums(clust_data[pam.res_k3$clustering==2,]),cex=1, main = "Cluster 2")
pie(colSums(clust_data[pam.res_k3$clustering==3,]),cex=1, main = "Cluster 3")
```

The pie charts show that the cluster least likely to make a purchase (cluster 1) has the most diverse interests, while the cluster most likely to make a purchase (cluster 3) has the least diverse interests. This trend was also observed in the PAM k=2 model before. Cluster 1 has the greatest interest in spending time in the web pages related to administration, such as account setup. Cluster 2 also has an interest in the Administrative pages as well, but not as much as cluster 3. Cluster 1 had the least interest in the Administrative pages among the three clusters. In summary, cluster 1 has the most diverse and varied interest, represented by many bigger pieces in the pie chart, cluster 2 has less diverse interest, and cluster 3 has the least one.

The following shows the medoids for cluster 1, 2, and 3 for the PAM model, respectively, as well as the medoids difference between the clusters.
...
Observations made in hierarchical clustering were similar to the previous observations derived in K-means and PAM. For instance, the cluster that was most likely to make a purchase (cluster 3 in this case), visited the Administrative websites the most, and also spent the most time in those websites. Cluster 1, which is the cluster least likely to make a purchase, visited these websites the least and also spent the least amount of time, while cluster 2 was in-between.
...
grid.arrange(hclustk3plot3, hclustk3plot4, hclustk3plot5, hclustk3plot6, nrow=2, ncol=2)
```

The same behaviour pattern was revealed for Informational and Product Related web pages as well.
...
The cluster 3 had the lowest Bounce and Exit rate, cluster 1 had the highest, while cluster 2 was in-between. This behaviour pattern was observed in the past models as well.

```{r, echo=FALSE}
#Explaining clusters by Page Values
ggplot(hresearchdata, aes(factor(groups_k3),y=PageValues)) +
  geom_boxplot(aes(fill=factor(groups_k3)))+coord_flip()+ggtitle("Page Values")
```

In all past observations, the cluster that was most likely going to make a purchase also had the highest Page value. However, the Hierarchical clustering with k=3 model performs differently in this case, because the cluster that was in-between has the highest Page value.

```{r, echo=FALSE}
#Explaining clusters by Months visits
ggplot(data = hresearchdata, mapping = aes(x = groups_k3, fill = as.factor(Month))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Month")
```

This plot shows that cluster 3 is most likely to shop around November, and cluster 1 is most likely to shop in the earlier months of the year. Cluster 2 shops more or less consistently throughout the year.
...
This plot shows the Operating System and Browser type used by three clusters. It seems that Operating System type 2 and Browser type 2 are the most utilized and popular across all clusters. Out of all three clusters, cluster 3 (most likely to make a purchase) is most likely to use a widely popular Operating System and Browser type, while cluster 1 is most likely to use an unpopular Operating System and Browser type.

```{r, echo=FALSE}
#Explaining clusters by Visitor Types
ggplot(data = hresearchdata, mapping = aes(x = groups_k3, fill = as.factor(VisitorType)))+geom_bar(position="fill")+ylab("Frequency")+ggtitle("Visitor Type") #+facet_wrap(groups_k3)+ coord_polar("y", start=0)
```

All three clusters are unlikely to be a Returning visitor to the website. However, cluster 3 is the most likely to be a New visitor to the website, while cluster 1 is the most likely to be "Other" visitor type. This trend was demonstrated in previous models as well.

```{r, echo=FALSE}
#Explaining clusters by Traffic Types
ggplot(data = hresearchdata, mapping = aes(x = groups_k3, fill = as.factor(TrafficType))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Traffic Type")
```

In all three clusters, Traffic type 3 is the most utilized and popular. Cluster 3 is most likely to use this popular Traffic type, while cluster 1 is likely to use more unpopular Traffic types to arrive at shopping websites.

```{r, echo=FALSE}
#Explaining clusters by Region types
hclustk3plot13<-ggplot(data = hresearchdata, mapping = aes(x = groups_k3, fill = as.factor(Region))) +geom_bar(position="fill")+ylab("Frequency")+ggtitle("Region")
#Explaining clusters by Weekend and Weekday visits
...
grid.arrange(hclustk3plot13, hclustk3plot16, nrow=2)
```

Clusters show no difference when it comes to Region and Weekend features, which is a similar result from previous models as well.


### Model Evaluation - Clustering, k=3

The same metrics were used again to compare which clustering model performed better in clustering for k=3. Silhouette score and a number of misplaced observations were utilized to identify the best clustering algorithm.

The following list shows how three different clustering models performed:
...
* k=3 misplaced observations: 982
...
* k=3 misplaced observations: 883
...
* k=3 misplaced observations: 1,264
...
rownames(metric_k3) <- c("Silhouette score","k=3 misplaced observations")
...
metrics_k3_clustering["k=3 misplaced observations","kmeans"]=0.982 #note: scaled
metrics_k3_clustering["k=3 misplaced observations","PAM"]=0.883 #note: scaled
metrics_k3_clustering["k=3 misplaced observations","HClust"]=1.264 #note: scaled
...
It seems that the PAM model again performed the best in Silhouette score, and also misplaced the least number of observations.
...
if (mean(k3_sil[,3])==best_sil) {"kmeans"} else if (pam.res_k3$silinfo$avg.width==best_sil) 
  {"pam"} else {"Hierarchical"}
```
...
PCA was performed, and produced the above graph, which shows the PCA number in the x-axis, and the variation representation in the y-axis.

The graph shows that PCA1 and PCA2 represent the majority of data variation.
...
PCA1 and PCA2 were plotted on the graph as above
...
The shopping pages should be designed to capture site-visitors’ interest to increase their stay, and thereby decreasing the Bounce and Exit Rate. Page value can be increased either by increasing the sale price of an item, or by decreasing the web page traffic required to arrive at the target/ transaction website (Google, 2020c).
...
R Documentation. (n.d.-a). Rand.index function | R Documentation. R Documentation. Retrieved May 12, 2020, from https://www.rdocumentation.org/packages/fossil/versions/0.3.7/topics/rand.index

R Documentation. (n.d.-b). ROSE function. Retrieved May 8, 2020, from https://www.rdocumentation.org/packages/ROSE/versions/0.0-3/topics/ROSE
...
Wikipedia. (2020). Rand index. In Wikipedia. https://en.wikipedia.org/w/index.php?title=Rand_index&oldid=954911959
